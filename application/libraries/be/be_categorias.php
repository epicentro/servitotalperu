...
class be_categorias {
    ...
    public function html_mnto($tabla, $idtabla, $carpeta_imagen){
            ...
            $strsql="select $idtabla, nombre, orden, idtipo_categoria, imagen  from $tabla 
                where padre_id='0' order by  orden, $idtabla ";
            ...
            if( $filas!="0" ){
                ...
                foreach($filas as $fila){
                            ...
                            if($fila->idtipo_categoria==1){
                                    $tipo="Categoría";    
                            }elseif($fila->idtipo_categoria==2){
                                    $tipo="<span class='seccion'>Sección</span>";  
                            }elseif($fila->idtipo_categoria==3){
                                    $tipo="<span class='especial'>Especial</span>";          
                            }//endIF
    ...
    public function html_combo($idselection, $tabla, $idtabla, $filtro){
        
       $parametro="";
        if(strlen($filtro)>0){
           $parametro=" and idtipo_categoria in (".$filtro.")";
           
       }
        
        $strsql="select $idtabla, nombre  from $tabla 
                where padre_id='0' $parametro  order by orden ";
    ...
    public function recursivo_migajas_li($id){
        
                     $strsql="select idcategoria_productos, nombre, seo, padre_id from categoria_productos where idcategoria_productos='$id'";
                     $rn=$this->CI->modelo_base->c_una_fila($strsql);
                     
                     if($rn!="0"){
                       return $this->recursivo_migajas_li($rn->padre_id)." <li class='breadcrumb-item'><a href='".base_url()."categoria/".$rn->seo."'>".$rn->nombre."</a></li>";
                     }//endIf
                     
    }//endFunction
    
    
    public function recursivo_padre_origen($id){
        
                     $strsql="select idcategoria_productos, nombre, seo, padre_id from categoria_productos where idcategoria_productos='$id'";
                     $rn=$this->CI->modelo_base->c_una_fila($strsql);
                     
                     if($rn!="0"){
                       return $this->recursivo_padre_origen($rn->padre_id).$rn->idcategoria_productos." ";
                     }//endIf
                     
    }//endFunction
    
    public function recursivo_ubicacion_para_seo($id){
        
        
       
            
                     $strsql="select idcategoria_ubicacion, nombre, padre_id from categoria_ubicacion where idcategoria_ubicacion='$id'";
                     $rn=$this->CI->modelo_base->c_una_fila($strsql);
                     
                     if($rn!="0"){
                        $ori = array("ñ", "á", "é", "í", "ó", "ú", "Á", "É", "Í", "Ó", "Ú", " ");
                        $new   = array("n", "a", "e", "i", "o", "u", "A", "E", "I", "O", "U", "-"); 
                        return str_replace($ori, $new, $rn->nombre)."-".$this->recursivo_ubicacion_para_seo($rn->padre_id);
                     }//endIf
