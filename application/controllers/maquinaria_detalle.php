 ...
 * @author Yasser
 */
class maquinaria_detalle extends padre {
    //put your code here
    ...
    public function index(){
        ...
        $strsql="select idmaquinarias, title, keyword, description, nombre,  descripcion, seo,idcategoria_maquinarias,video,sumilla from maquinarias where seo='$seo'";
        
        $fila=$this->modelo_base->c_una_fila($strsql);
        
        if($fila!="0"){
        
                $data["idmaquinarias"]=$fila->idmaquinarias;
                $data["title"]=$fila->title;
                $data["keyword"]=$fila->keyword;
                $data["description"]=$fila->description;
                $data["titulo"]=$fila->nombre;
                $data["descripcion"]=$fila->descripcion;
                $data["idcategoria_maquinarias"]=$fila->idcategoria_maquinarias;                
    ...
    {
            $idmaquinarias=$this->input->post('idmaquinarias');
            ...
            */
            if($check == 1)
            {
               // echo 'Validación pasada correctamente';
                ...
                $strsql="insert into comentarios (idmaquinarias, nombre, email, pregunta, fecha, idsw) "
                        . "VALUES ('$idmaquinarias', '$nombre', '$email',$comentario,'$fecha','1')";
